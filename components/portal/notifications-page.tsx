...
const TYPE_META: Record<string, { icon: React.ReactNode; tone: string; label: string }> = {
  ...
  "issue-status": {
    ...
    tone: "bg-teal-50 text-teal-600 ring-teal-200",
    label: "Status",
  },
};
...
export function PortalNotificationsPage({
  ...
  const items = useMemo(
    () => (data?.notifications ?? []).filter((n) => !hidden.includes(n.id)),
    [data, hidden],
  );

  const isRead = (id: string, baseRead: boolean) => readOverrides[id] ?? baseRead;
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4 pb-6">
      <PortalPageHeader
        ...
        iconBg={accent}
        title={title}
        subtitle={`${unread} ${subtitlePrefix}${loading ? " · syncing" : ""}`}
        action={
          items.length > 0 ? (
      ...
      {items.length === 0 && !loading ? (
        <div className="glass flex flex-col items-center gap-2 rounded-2xl p-8 text-center shadow-lg shadow-slate-900/5 ring-1 ring-white/60">
          <Bell size={28} className="text-slate-400" />
          <p className="text-sm font-bold text-slate-900">No notifications yet</p>
          <p className="text-xs text-slate-700">
            Status changes on issues you care about will land here in real time.
          </p>
        </div>
      ) : (
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-2">
                    <p className={cn("text-sm font-bold text-slate-900", isReadFlag && "text-slate-700")}>
                      {n.title}
                      {!isReadFlag && n.type === "alert" && (
                        <span className="ml-2 rounded-full bg-red-50 px-2 py-0.5 text-[9px] font-bold text-red-600 ring-1 ring-red-200">
                          URGENT
                        </span>
                      )}
                    </p>
                    <span className="shrink-0 text-[10px] font-semibold text-slate-700">{timeAgoLabel(n.createdAt)}</span>
                  </div>
