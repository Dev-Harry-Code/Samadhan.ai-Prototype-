...
export function CompanyInsightsPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-5 pb-6">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {[
          ...
          <div key={k.label} className="glass rounded-2xl p-4 shadow-lg shadow-slate-900/5 ring-1 ring-white/60">
            <k.icon size={20} className="text-slate-500" />
      ...
      <div className="glass rounded-3xl bg-gradient-to-br from-slate-900 via-emerald-950 to-teal-900 p-6 text-white shadow-lg shadow-slate-900/10 ring-1 ring-white/10">
        <p className="text-sm font-bold">AI Recommendation of the day</p>
        <p className="mt-3 max-w-2xl text-sm leading-relaxed text-emerald-50/90">
          Pre-schedule <strong className="text-amber-300">preventive patching on West Lane</strong> before tomorrow&apos;s
          rain band. Historical data suggests this corridor produces 12–15 fresh pothole reports after each monsoon
          spell — proactive repair could cut that by <strong className="text-emerald-300">~40%</strong>.
        </p>
          ...
          <div className="mt-2 flex h-40 items-center gap-4">
            ...
            <div className="space-y-2">
              {COMPANY_ANALYTICS.sentimentPie.map((s, i) => (
                <div key={s.name} className="flex items-center gap-1.5 text-xs font-semibold text-slate-600">
                  <span
          ...
          {INSIGHTS.map((item) => (
            <div key={item.title} className="group rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-900/5 transition hover:bg-white hover:shadow-md">
              <div className="flex items-start gap-3">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white text-slate-500 ring-1 ring-slate-900/5">
                  <item.icon size={18} />
                </span>
                <div>
          ...
          {critical.map((i) => (
            <Link key={i.id} href={`/funder/issues/${i.id}`} className="group flex flex-wrap items-center gap-3 rounded-2xl bg-slate-50 p-3 ring-1 ring-slate-900/5 transition hover:bg-white hover:shadow-md">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-slate-600">{i.id} · {i.area}</p>
              </div>
              <span className="rounded-full bg-red-50 px-2.5 py-0.5 text-[11px] font-bold text-red-600">
                {i.status === "open" ? "Unassigned" : i.assignee}
              </span>
              <ArrowRight size={15} className="text-slate-500 transition group-hover:text-red-500" />
            </Link>
          ))}
        </div>
      </div>
